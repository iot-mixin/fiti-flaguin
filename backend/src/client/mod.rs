    ...
    C: Has<XSpanIdString> + Has<Option<AuthData>> + Clone + Send + Sync + 'static,
{
    ...
    async fn create_phrase_by_id(
        &self,
        param_phrase_id: uuid::Uuid,
        param_phrase: Option<models::Phrase>,
        context: &C) -> Result<CreatePhraseByIdResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/api/v1/phrases/{phrase_id}",
            self.base_path
            ,phrase_id=utf8_percent_encode(&param_phrase_id.to_string(), ID_ENCODE_SET)
        );
        ...
        // Body parameter
        let body = param_phrase.map(|ref body| {
        ...
        match response.status().as_u16() {
            201 => {
                ...
                let body = serde_json::from_str::<models::Phrase>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(CreatePhraseByIdResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
                Ok(
                    CreatePhraseByIdResponse::InvalidIDSupplied
                )
            }
            404 => {
                Ok(
                    CreatePhraseByIdResponse::OrderNotFound
                )
            }
            code => {
        ...
        context: &C) -> Result<DeletePhraseByIdResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            204 => {
                Ok(
                    DeletePhraseByIdResponse::SuccessfulOperation
                )
            }
            code => {
                let headers = response.headers().clone();
        ...
        context: &C) -> Result<GetPhraseByIdResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(GetPhraseByIdResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
                Ok(
                    GetPhraseByIdResponse::InvalidIDSupplied
                )
            }
            404 => {
                Ok(
                    GetPhraseByIdResponse::OrderNotFound
                )
            }
            code => {
                let headers = response.headers().clone();
        ...
        context: &C) -> Result<GetPhrasesResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<Vec<models::Phrase>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GetPhrasesResponse::SuccessfulOperation
                    (body)
                )
            }
            code => {
